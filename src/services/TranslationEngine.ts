...
export class TranslationEngine implements ITranslationEngine {
  ...
  constructor(
    ...
    this.lexiconService = lexiconService || new LexiconService();
    this.safetyValidator = safetyValidator || new SafetyValidator();
    this.regionalAdapter = regionalAdapter || new RegionalAdapter();
  }
   ...
   */
  private createUnknownTermResult(input: string, context?: RegionalContext): TranslationResult {
    const addressing = context ? 
      this.regionalAdapter.getRegionalAddressing(context.region) : 
      'Dada';
   ...
   */
  private createBhaiyaCorrectionResult(input: string, context?: RegionalContext): TranslationResult {
   ...
   */
  private createEmptyResult(message: string): TranslationResult {
    return {
      term: null,
   ...
   */
  private generateCulturalNotes(term: SlangTerm, context?: RegionalContext): string[] {
    const notes: string[] = [];
   ...
   */
  private generatePersonalityElements(term: SlangTerm, context?: RegionalContext): PersonalityElement[] {
